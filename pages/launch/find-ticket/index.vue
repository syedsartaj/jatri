    ...
    <div class="w-full lg:w-[816px] mx-auto pt-6 pb-10 md:pt-12 lg:pb-[100px]">
      <div class="w-full bg-white rounded-[10px] border border-[#DBDBDB]">
        ...
        <div class="w-full mt-6 p-4">
          <div class="flex  h-[40px] lg:h-[36px] justify-between rounded-t-[10px] max-w-max ml-4 bg-[#f7f7f7]">
            <div :class="getServiceClassName(ServiceType.BUS)" @click="handleServiceChange(ServiceType.BUS)">
              <svg class="h-[20px] w-[20px] lg:h-[24px] lg:w-[24px]" width="20" height="19" viewBox="0 0 20 19"
                ...
                <path
                  ...
                  :fill="selectedService === ServiceType.BUS ? 'white' : '#151414'
                    " />
              </svg>

              <span :class="selectedService === ServiceType.BUS
                  ? 'text-white'
                  : 'text-black'
                " class="text-xs lg:text-xl font-medium ml-[10px]">Bus</span>
            </div>
            <div :class="getServiceClassName(ServiceType.LAUNCH)" @click="handleServiceChange(ServiceType.LAUNCH)">
              <svg class="h-[20px] w-[20px] lg:h-[24px] lg:w-[24px]" width="23" height="21" viewBox="0 0 23 21"
                fill="none" xmlns="http://www.w3.org/2000/svg">
                <path
                  ...
                  :fill="selectedService === ServiceType.LAUNCH ? 'white' : '#151414'
                    " />
              </svg>

              <span :class="selectedService === ServiceType.LAUNCH
                  ? 'text-white'
                  : 'text-black'
                " class="text-xs lg:text-xl font-medium ml-[10px]">Launch</span>
            </div>
          </div>
          <div class="w-full bg-white rounded-[8px] border border-[#DBDBDB] p-4">
            <div class="flex gap-x-[8px]">
              <button @click="selectedTab = 0"
                class="px-[11px]  py-[5px] rounded-full border text-sm font-medium lg:px-6" :class="selectedTab === 0
              ...
              <button @click="selectedTab = 1"
                class="px-[10px]  py-[5px] rounded-full border text-sm font-medium lg:px-6" :class="selectedTab === 1
                    ? 'bg-[#F04935] border-[#F04935] text-white'
                    : 'bg-white text-blackPrimary border-[#DBDBDB]'
                  ">
                PNR
              ...
              <button @click="selectedTab = 2"
                class="px-[11px]  py-[5px] rounded-full border text-sm font-medium lg:px-6" :class="selectedTab === 2
                    ? 'bg-[#F04935] border-[#F04935] text-white'
                    : 'bg-white text-blackPrimary border-[#DBDBDB]'
                  ">
                Tnx ID
            ...
            <p class="mt-5 lg:mt-4 mb-[10px] text-xs font-medium text-blackPrimary">
              {{
                selectedTab === 0
                  ? "Mobile Number"
                  : selectedTab === 1
            ...
            <form @submit="ticketData">
              <div class="mt-[10px] flex flex-col md:flex-row justify-center md:justify-between gap-x-14">
                <div class="w-full md:w-9/12">
                  <div v-if="selectedTab === 0" class="flex px-2 py-[6px]  bg-[#F7F7F7] rounded md:p-4">
                    <div class="flex items-center shrink-0">
                      <img class="w-[32px] h-[32px]" src="@/assets/images/bd-flag.svg" alt="" />
                      <div
                        class="text-[14px] leading-[24px] font-Inter font-[400] tracking-wide text-[#747476] ml-[10px]">
                        +880
                      </div>
                      <img class="w-[2px] h-[24px] ml-[4px] mr-2" src="@/assets/images/input-separator.svg" alt="" />
                    </div>
                    <input
                      ...
                      type="number" required="" placeholder="Enter Mobile Number" v-model="phone" @input="handleInput"
                      @paste="handlePaste" @wheel="$event.target.blur()" />
                  </div>
                  <input v-else-if="selectedTab === 1"
                    ...
                    type="text" required="" placeholder="PNR" v-model="pnr" />
                  <input v-else-if="selectedTab === 2" required=""
                    class="w-full p-4 bg-[#F7F7F7] rounded outline-none text-sm lg:text-base placeholder:text-sm lg:placeholder:text-base font-normal text-blackPrimary placeholder:text-[#676769]"
                    type="text" placeholder="Tnx ID" v-model="transactionId" />
                </div>
                <button type="submit"
          ...
          <div class="flex justify-center mt-[10px]">
            <div v-if="alertMessage && !getSearchedTicketList.tickets"
              class="w-full flex flex-row gap-x-2 items-center text-xs md:text-sm font-normal text-[#E0293B]">
              <img src="@/assets/images/icons/warningRed.svg" class="h-[16px] w-[16px] md:h-6 nd:w-6" alt="error" />
      ...
      <div v-if="getSearchedTicketList.tickets">
        <div class="w-full flex flex-col" v-if="getActiveTickets?.length">
          ...
          <div v-for="ticket in getActiveTickets" :key="ticket._id" class="mt-6">
            <SingleTicketListItem :singlePrintTicketInfo="ticket" :serviceType="selectedService" />
          </div>
        </div>
