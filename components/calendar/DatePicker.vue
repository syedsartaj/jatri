<template>
  <div class="w-1/4">
    <button class="block rounded-lg bg-white focus:outline-none w-full lg:px-5 xl:px-6 py-4 cursor-pointer" @click="toggleDropdown">
      ...
      <p v-if="dateUpdate !== ''" class="lg:text-xs xl:text-sm font-medium text-blackPrimary text-left"> {{ new Date(dateUpdate).toLocaleString("es-CL", { dateStyle: "medium", }) }} </p>
      <p v-else-if="defaultOption" class="lg:text-xs xl:text-sm font-medium text-blackPrimary text-left"> 
        {{ defaultOption }}  
      </p>
